...
const navItems = [
  ...
  { to: '/owner', icon: Users, label: '业主管理' },
  { to: '/employee', icon: Building2, label: '员工管理' },
  { to: '/house', icon: Home, label: '房屋管理' },
  { to: '/fee', icon: Receipt, label: '费用管理' },
  { to: '/parking', icon: Car, label: '停车位管理' },
  { to: '/complaint', icon: MessageSquare, label: '投诉管理' },
  { to: '/repair', icon: Wrench, label: '报修管理' },
  { to: '/duty', icon: CalendarClock, label: '值班管理' },
]
    ...
    <div className="flex h-screen overflow-hidden bg-bg-primary">
      ...
      >
        <div className="h-16 flex items-center justify-between px-5 border-b border-white/[0.06]">
          ...
          <button onClick={() => setMobileOpen(false)} className="p-1.5 rounded-lg hover:bg-white/[0.06] text-text-secondary">
            <X size={18} />
          </button>
        </div>
        <nav className="flex-1 py-3 overflow-y-auto px-2 space-y-0.5">
        ...
        <div className="p-3 border-t border-white/[0.06]">
          <div className="flex items-center gap-3 px-2 py-2">
            ...
            <button onClick={handleLogout} className="p-1.5 rounded-lg hover:bg-danger/10 text-text-secondary hover:text-danger transition-colors">
              <LogOut size={15} />
            </button>
          </div>
        </div>
      ...
      <main ref={mainRef} className="flex-1 overflow-y-auto relative z-10">
        ...
        <div className="lg:hidden sticky top-0 z-20 flex items-center h-14 px-4 border-b border-white/[0.06]" style={{ background: 'rgba(10, 10, 15, 0.85)', backdropFilter: 'blur(12px)' }}>
          <button onClick={() => setMobileOpen(true)} className="p-1.5 rounded-lg hover:bg-white/[0.06] text-text-secondary">
            <Menu size={20} />
          </button>
